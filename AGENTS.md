...
- Generate visual assets with the `asset-gen` skill (`$asset-gen` in Codex). Confirm the spend with the user before the first paid generation.
...
- Skills: `.cursor/skills/asset-gen` (Cursor), `.claude/skills/asset-gen` (Claude Code), `.agents/skills/asset-gen` (Codex).
...
- Asset CLI (from repo root): `.\.venv\Scripts\python.exe .cursor/skills/asset-gen/tools/asset_gen.py image --prompt "..." -o assets/...`
